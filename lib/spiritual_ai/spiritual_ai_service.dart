...
class SpiritualAiService {
  Future<SpiritualAiReply> ask({
    ...
    if (retriever.wantsHistoricalFacts(trimmed)) {
      askMessage =
          '$askMessage\n\n(Համակարգ. օգտատերը ուզում է պատմական տվյալներ։ Հայերենով կարճ ասա ժամանակը, վայրը և ինչ է եղել։ Եթե Աստվածաշնչի գիրք է, պարտադիր ասա նաև ում համար է գրվել և ինչու է գրվել։ Թվեր մի հորինիր։ Անգլերեն բառ մի գրիր։ Նկար մի խոստացիր։)';
